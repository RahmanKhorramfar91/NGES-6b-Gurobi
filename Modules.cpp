...
#pragma region Electricity Network Constraints
	...
	for (int n = 0; n < nEnode; n++)
	{
		for (int t = 0; t < Te.size(); t++)
		{
			...
			{
				//Model.addConstr(EV::prod[n][t][i] >= Plants[i].Pmin * EV::Xop[n][i]); since we don't consider unit commitment in this model
				Model.addConstr(EV::prod[n][t][i] <= Plants[i].Pmax * EV::val_Xop[n][i]);

				//if (t > 0 && !Setting::heuristics1_active) 
				if (t > 0)
				{
					Model.addConstr(-Plants[i].rampU * Plants[i].Pmax * EV::val_Xop[n][i] <= EV::prod[n][t][i] - EV::prod[n][t - 1][i]);
					Model.addConstr(EV::prod[n][t][i] - EV::prod[n][t - 1][i] <= Plants[i].rampU * Plants[i].Pmax * EV::val_Xop[n][i]);
				}
			}
		}
	}
	...
	for (int br = 0; br < nBr; br++)
	{
		for (int t = 0; t < Te.size(); t++)
		{
			...
			{
				Model.addConstr(EV::flowE[br][t] <= Branches[br].maxFlow * EV::val_Ze[br]);
				Model.addConstr(-Branches[br].maxFlow * EV::Ze[br] <= EV::flowE[br][t]);
			}
		}
	}
	...
	for (int br = 0; br < Branches.size(); br++)
	{
		...
		for (int t = 0; t < Te.size(); t++)
		{
			...
			{
				// Big M = Branches[br].suscep * 200
				Model.addConstr(EV::flowE[br][t] - Branches[br].suscep * (EV::theta[tb][t] - EV::theta[fb][t]) <= 1e-2 + Branches[br].suscep * 200 * (1 - EV::val_Ze[br]));
				Model.addConstr(-1e-2 - Branches[br].suscep * 200 * (1 - EV::val_Ze[br]) <= EV::flowE[br][t] - Branches[br].suscep * (EV::theta[tb][t] - EV::theta[fb][t]));
			}
		}
	}
	...
	for (int n = 0; n < nEnode; n++)
	{
		for (int t = 0; t < Te.size(); t++)
		{
			for (int i = 0; i < nPlt; i++)
			{
				if (Plants[i].type == "solar" | Plants[i].type == "solar-UPV")
				{
					Model.addConstr(EV::prod[n][t][i] <= Plants[i].zonal_profile[Te[t]][n] * Plants[i].Pmax * EV::val_Xop[n][i]);
				}
				if (Plants[i].type == "wind" || Plants[i].type == "wind-new")
				{
					Model.addConstr(EV::prod[n][t][i] <= Plants[i].zonal_profile[Te[t]][n] * Plants[i].Pmax * EV::val_Xop[n][i]);
				...
				if (Plants[i].type == "wind_offshore" || Plants[i].type == "wind-offshore-new")
				{
					Model.addConstr(EV::prod[n][t][i] <= Plants[i].zonal_profile[Te[t]][0] * Plants[i].Pmax * EV::val_Xop[n][i]);
				}
			}
		}
	}
