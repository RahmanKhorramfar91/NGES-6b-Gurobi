...
#pragma region Electricity network DVs
	...
	for (int b = 0; b < nBr; b++)
	{
		EV::flowE[b] = Model.addVars(Params::Te.size());
	...
	{
		EV::PB[n] = new GRBConstr[Params::Te.size()];
		if (Setting::relax_int_vars)
		{
		...
		else
		{
			...
			{
				if (Params::Plants[i].type == "solar-UPV" || Params::Plants[i].type == "wind-new")
				{
		...
		EV::theta[n] = Model.addVars((int)Params::Te.size(), GRB_CONTINUOUS);
		EV::curtE[n] = Model.addVars((int)Params::Te.size(), GRB_CONTINUOUS);
		...
		EV::prod[n] = new GRBVar * [Params::Te.size()];
		EV::eSch[n] = new GRBVar * [Params::Te.size()];
		EV::eSdis[n] = new GRBVar * [Params::Te.size()];
		EV::eSlev[n] = new GRBVar * [Params::Te.size()];
...
#pragma region NG network DVs
	...
	{
		// uni-directional
		GV::flowGG[i] = Model.addVars(Params::Tg.size(), GRB_CONTINUOUS);
	}
	for (int j = 0; j < nSVL; j++)
	{
		GV::Sstr[j] = Model.addVars(Params::Tg.size(), GRB_CONTINUOUS);
		GV::Svpr[j] = Model.addVars(Params::Tg.size(), GRB_CONTINUOUS);
		GV::Sliq[j] = Model.addVars(Params::Tg.size(), GRB_CONTINUOUS);
		GV::flowVG[j] = new GRBVar * [nGnode];
		for (int k = 0; k < nGnode; k++)
		{
			GV::flowVG[j][k] = Model.addVars(Params::Tg.size(), GRB_CONTINUOUS);
		}
	}

	for (int k = 0; k < nGnode; k++)
	{
		GV::supply[k] = Model.addVars(Params::Tg.size(), GRB_CONTINUOUS);
		GV::curtG[k] = Model.addVars(Params::Tg.size(), GRB_CONTINUOUS);
		GV::curtRNG[k] = Model.addVars(Params::Tg.size(), GRB_CONTINUOUS);
		GV::flowGE[k] = new GRBVar * [nEnode];
		GV::flowGL[k] = new GRBVar * [nSVL];
		for (int j = 0; j < nSVL; j++)
		{
			GV::flowGL[k][j] = Model.addVars(Params::Tg.size(), GRB_CONTINUOUS);

		}
		for (int n = 0; n < nEnode; n++)
		{
			GV::flowGE[k][n] = Model.addVars(Params::Tg.size(), GRB_CONTINUOUS);
		}
	}
#pragma endregion
...
#pragma region Set some variables
	...
	for (int n = 0; n < nEnode; n++)
	{
		for (int i = 0; i < nPlt; i++)
		{
			...
			if (Params::Plants[i].type == "hydro-new")
			{
				Model.addConstr(EV::Xest[n][i] == 0);
			}
			if (Params::Plants[i].type == "wind-offshore-new")
			{
			...
			if (Params::Plants[i].type == "nuclear-new")
			{
				Model.addConstr(EV::Xest[n][i] == 0);
			}

			if (Params::Plants[i].is_exis == 1)
			{
				Model.addConstr(EV::Xest[n][i] == 0);
			}
			else
...
#pragma region Objective Function
	...
	for (int n = 0; n < nEnode; n++)
	{
		for (int i = 0; i < nPlt; i++)
		{
			...
			double s1 = (double)std::pow(1.0 / (1 + WACC), Params::Plants[i].lifetime);
			double capco = WACC / (1 - s1) * Params::Plants[i].Reg_coeffs_per_state[n];
			ex_est += capco * Params::Plants[i].capex * EV::Xest[n][i];
			ex_decom += Params::Plants[i].decom_cost * EV::Xdec[n][i];
		}
		...
		for (int i = 0; i < nPlt; i++)
		{
			...
			ex_fix += Params::Plants[i].fix_cost * EV::Xop[n][i];
		}
	...
	for (int b = 0; b < Params::Branches.size(); b++)
	{
		...
		double capco = WACC / (1 - s1);
		int fb = Params::Branches[b].from_bus;
		int tb = Params::Branches[b].to_bus;
		// find the index of tb in the adj_buses of the fb
		int tbi = std::find(Params::Enodes[fb].adj_buses.begin(), Params::Enodes[fb].adj_buses.end(), tb) - Params::Enodes[fb].adj_buses.begin();
		ex_trans += capco * trans_unit_cost * Params::Branches[b].maxFlow * Params::Branches[b].length * EV::Ze[b];
	}
...
#pragma region Electricity Network Constraints
	...
	for (int n = 0; n < nEnode; n++)
	{
		for (int i = 0; i < nPlt; i++)
		{
			...
			bool isInd = std::find(Params::Enodes[n].Init_plt_ind.begin(), Params::Enodes[n].Init_plt_ind.end(), i) != Params::Enodes[n].Init_plt_ind.end();
			if (isInd)
			{
				// find the index of plant in the set of Init_plants of the node
				string tp = pltType2sym[i];
				int ind1 = std::find(Params::Enodes[n].Init_plt_types.begin(), Params::Enodes[n].Init_plt_types.end(), tp) - Params::Enodes[n].Init_plt_types.begin();
				Model.addConstr(EV::Xop[n][i] == Params::Enodes[n].Init_plt_count[ind1] - EV::Xdec[n][i] + EV::Xest[n][i]);
			}
			else
			{
	...
	for (int n = 0; n < nEnode; n++)
	{
		for (int t = 0; t < Params::Te.size(); t++)
		{
			for (int i = 0; i < nPlt; i++)
			{
				...
				Model.addConstr(EV::prod[n][t][i] <= Params::Plants[i].Pmax * EV::Xop[n][i]);

				//if (t > 0 && !Setting::heuristics1_active) 
				if (t > 0)
				{
					Model.addConstr(-Params::Plants[i].rampU * Params::Plants[i].Pmax * EV::Xop[n][i] <= EV::prod[n][t][i] - EV::prod[n][t - 1][i]);
					Model.addConstr(EV::prod[n][t][i] - EV::prod[n][t - 1][i] <= Params::Plants[i].rampU * Params::Plants[i].Pmax * EV::Xop[n][i]);
				}
			}
		}
	}
	...
	for (int br = 0; br < nBr; br++)
	{
		for (int t = 0; t < Params::Te.size(); t++)
		{
			...
			{
				Model.addConstr(EV::flowE[br][t] <= Params::Branches[br].maxFlow * EV::Ze[br]);
	...
	for (int br = 0; br < Params::Branches.size(); br++)
	{
		...
		for (int t = 0; t < Params::Te.size(); t++)
		{
			...
			else
			{
				...
				Model.addConstr(EV::flowE[br][t] - Params::Branches[br].suscep * (EV::theta[tb][t] - EV::theta[fb][t]) <= 1e-2 + Params::Branches[br].suscep * 200 * (1 - EV::Ze[br]));
				Model.addConstr(-1e-2 - Params::Branches[br].suscep * 200 * (1 - EV::Ze[br]) <= EV::flowE[br][t] - Params::Branches[br].suscep * (EV::theta[tb][t] - EV::theta[fb][t]));
			}
		}
	}
	...
	for (int n = 0; n < nEnode; n++)
	{
		for (int t = 0; t < Params::Te.size(); t++)
		{
			for (int i = 0; i < nPlt; i++)
			{
				...
				{
					Model.addConstr(EV::prod[n][t][i] <= Params::Plants[i].zonal_profile[Params::Te[t]][n] * Params::Plants[i].Pmax * EV::Xop[n][i]);
				}
				...
				{
					Model.addConstr(EV::prod[n][t][i] <= Params::Plants[i].zonal_profile[Params::Te[t]][0] * Params::Plants[i].Pmax * EV::Xop[n][i]);
				}
			}
		}
	}
			...
			for (int r = 0; r < neSt; r++)
			{
				...
				if (t == 0)
				{
					Model.addConstr(EV::eSlev[n][t][r] ==
						Params::Estorage[r].eff_ch * EV::eSch[n][t][r] -
						(EV::eSdis[n][t][r] / Params::Estorage[r].eff_disCh));
				}
				else
				{
	...
	for (int n = 0; n < nEnode; n++)
	{
		for (int r = 0; r < neSt; r++)
		{
			for (int k = 0; k < Params::Tg.size(); k++)
			{
				int str = k * 24;
				int end = (k + 1) * 24 - 1;
...
#pragma region NG network related costs
	...
	for (int k = 0; k < nGnode; k++)
	{
		...
		{
			exp_NG_import_cost += Params::RepDaysCount[tau] * GV::supply[k][tau] * NG_price;
		}
	}
	...
	for (int j = 0; j < nSVL; j++)
	{
		...
		double capco = WACC / (1 - s1);
		exp_invG += capco * (Params::SVLs[0].Capex * GV::Xstr[j] + Params::SVLs[1].Capex * GV::Xvpr[j]);
	}

	// storage FOM cost
	for (int j = 0; j < nSVL; j++)
	{
		exp_StrFOM += Params::SVLs[0].FOM * (Params::Exist_SVL[j].vap_cap + GV::Xvpr[j]);
		exp_StrFOM += Params::SVLs[1].FOM * (Params::Exist_SVL[j].store_cap + GV::Xstr[j]);
	}
	...
	for (int k = 0; k < nGnode; k++)
	{
		for (int tau = 0; tau < Params::Tg.size(); tau++)
		{
			exp_ng_curt += Params::RepDaysCount[tau] * G_curt_cost * GV::curtG[k][tau];
			exp_rng_curt += Params::RepDaysCount[tau] * Params::RNG_price * GV::curtRNG[k][tau];
		}
	}
...
#pragma region NG Network Constraint
	...
	for (int i = 0; i < nPipe; i++)
	{
		int is_exist = Params::PipeLines[i].is_exist;
		for (int tau = 0; tau < Params::Tg.size(); tau++)
		{
			if (is_exist == 1)
			{
				Model.addConstr(GV::flowGG[i][tau] <= Params::PipeLines[i].cap);
			}
			else
			{
				Model.addConstr(GV::flowGG[i][tau] <= Params::PipeLines[i].cap * GV::Zg[i]);
			}
		}
	}
	...
	for (int k = 0; k < nGnode; k++)
	{
		for (int tau = 0; tau < Params::Tg.size(); tau++)
		{
			...
			for (int l : Params::Gnodes[k].Lexp) { exp_gg_exp += GV::flowGG[l][tau]; }
			for (int l : Params::Gnodes[k].Limp) { exp_gg_imp += GV::flowGG[l][tau]; }

			for (int n : Params::Gnodes[k].adjE) { exp_ge_flow += GV::flowGE[k][n][tau]; }
			for (int j : Params::Gnodes[k].adjS)
			{
			...
			double dem = Params::Gnodes[k].demG[Params::Tg[tau]];// +Gnodes[k].out_dem;
			total_dem += Params::RepDaysCount[tau] * dem;
	...
	for (int k = 0; k < nGnode; k++)
	{
		for (int tau = 0; tau < Params::Tg.size(); tau++)
		{
			Model.addConstr(GV::supply[k][tau] >= Params::Gnodes[k].injL);
			Model.addConstr(GV::supply[k][tau] <= Params::Gnodes[k].injU);
			double dem = Params::Gnodes[k].demG[Params::Tg[tau]];// +Gnodes[k].out_dem;
			Model.addConstr(GV::curtG[k][tau] + GV::curtRNG[k][tau] <= dem);
			exp_RNG += Params::RepDaysCount[tau] * GV::curtRNG[k][tau];
		}
	}
	...
	for (int j = 0; j < nSVL; j++)
	{
		for (int tau = 0; tau < Params::Tg.size(); tau++)
		{
			if (tau == 0)
			{
				...
				Model.addConstr(GV::Sstr[j][tau] == Params::Exist_SVL[j].store_cap * 0 + GV::Sliq[j][tau] - GV::Svpr[j][tau] / Params::SVLs[1].eff_disCh);
				//Model.addConstr(GV::Sstr[j][tau] == GV::Sliq[j][tau] - GV::Svpr[j][tau] / SVLs[1].eff_disCh);
				continue;
			}
			Model.addConstr(GV::Sstr[j][tau] == (1 - Params::SVLs[0].BOG) * GV::Sstr[j][tau - 1] + GV::Sliq[j][tau] - GV::Svpr[j][tau] / Params::SVLs[1].eff_disCh);
		}
	}
	...
	for (int j = 0; j < nSVL; j++)
	{
		for (int tau = 0; tau < Params::Tg.size(); tau++)
		{
			...
			for (int k = 0; k < nGnode; k++)
			{
				for (int j2 : Params::Gnodes[k].adjS)
				{
					if (j2 == j)
					{
						NG_adj.push_back(k);
	...
	for (int j = 0; j < nSVL; j++)
	{
		for (int tau = 0; tau < Params::Tg.size(); tau++)
		{
			Model.addConstr(GV::Sliq[j][tau] <= Params::Exist_SVL[j].liq_cap);
		}
	}
	...
	for (int j = 0; j < nSVL; j++)
	{
		for (int tau = 0; tau < Params::Tg.size(); tau++)
		{
			Model.addConstr(GV::Svpr[j][tau] <= Params::Exist_SVL[j].vap_cap + GV::Xvpr[j]);
		}
	}
	...
	for (int j = 0; j < nSVL; j++)
	{
		for (int tau = 0; tau < Params::Tg.size(); tau++)
		{
			Model.addConstr(GV::Sstr[j][tau] <= Params::Exist_SVL[j].store_cap + GV::Xstr[j]);
		}
	}
	...
	for (int k = 0; k < nGnode; k++)
	{
		for (int tau = 0; tau < Params::Tg.size(); tau++)
		{
			for (int n : Params::Gnodes[k].adjE)
			{
					...
					for (int i = 0; i < nPlt; i++)
					{
							...
							Params::Plants[i].type == "CC" || Params::Plants[i].type == "CC-CCS")
						{
							if (Setting::DGSP_active)
							{
								...
								exp2 += Params::time_weight[t] * Params::Plants[i].heat_rate * EV::val_prod[n][t][i];
								flowge += Params::time_weight[t] * Params::Plants[i].heat_rate * EV::val_prod[n][t][i];
							}
							else
							{
				...
				Model.addConstr(Params::RepDaysCount[tau] * GV::flowGE[k][n][tau] - exp2 <= 1e-2);
				Model.addConstr(exp2 - Params::RepDaysCount[tau] * GV::flowGE[k][n][tau] <= 1e-2);
				ex_xi += Params::RepDaysCount[tau] * GV::flowGE[k][n][tau];
			}
		}
	}
#pragma endregion
...
#pragma region Coupling 2
	for (int k = 0; k < nGnode; k++)
	{
		for (int tau = 0; tau < Params::Tg.size(); tau++)
		{
			ex_NG_emis += Params::RepDaysCount[tau] * NG_emis_rate * (GV::supply[k][tau]);
		}
	}
